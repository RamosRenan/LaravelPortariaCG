...
<script>
    ...
    var route = '{{ route('refectory.employees.mass_destroy') }}';
</script>
@stop
...
@section('content')
    <div class="card card-default">
        <div class="card-header">
            <div class="row">
                <div class="col-md-6">
                    <a href="{{ route('refectory.employees.create') }}" class="btn btn-sm btn-success"><i class="fa fa-plus"></i> @lang('global.app_create')</a>
                </div>
                <div class="col-md-6">
                    <div class="float-sm-right">
                        {{ Form::open(['method' => 'GET', 'route' => ['refectory.employees.index']]) }}
            ...
            <table class="table table-head-fixed table-hover">
                <thead>
                    <tr>
                        ...
                        <th>@lang('refectory.employees.fields.rg')</th>
                        <th>@lang('refectory.employees.fields.cpf')</th>
                        <th>@lang('refectory.employees.fields.name')</th>
                        <th>@lang('refectory.employees.fields.units')</th>
                        <th>@lang('refectory.employees.fields.specialty')</th>
                        <th>&nbsp;</th>
                    </tr>
                </thead>
                <tbody>
                @foreach ($items as $item)
                    <tr data-entry-id="{{ $item->id }}">
                        ...
                        <td class="align-middle">
                            @foreach ($item->units($item->id)->pluck('name') as $unit)
                                <span class="badge badge-info badge-many">{{ $unit }}</span>
                            @endforeach
                        </td>
                        <td class="align-middle">
                            @foreach ($item->specialties($item->id)->pluck('name') as $specialty)
                                <span class="badge badge-info badge-many">{{ $specialty }}</span>
                            @endforeach
                        </td>
                        <td class="align-middle text-right">
                            {{ Form::open([
                                ...
                                'method' => 'DELETE',
                                'route' => ['refectory.employees.destroy', $item->id]
                            ]) }}
                            <div class="btn-group">
                                <a href="{{ route('refectory.employees.edit',[$item->id]) }}" class="btn btn-sm btn-primary"><i class="fa fa-edit"></i> @lang('global.app_edit')</a>
